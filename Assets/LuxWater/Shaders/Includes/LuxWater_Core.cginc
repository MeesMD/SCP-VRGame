...
//FogDensity += FogDensity * (noise.x - 0.5) * 2 * 0.005;
    ...
    //  Using abs to get some translucency when rendering underwater
        // transDot *= saturate( (1.0 - abs(worldNormal.y + Foam.a * 0.0075    )) * 2 ); //* 5         );
        transDot *= saturate( (1.0 - lerp(abs( worldNormal.y), 1 - worldNormalFace.y, Foam.a * .035)    ) * 2 );

        lightScattering = transDot * lightColor * ColorAbsorption * fogLightCancellation; // * Translucency;  <-- unused

//      Add translucent lighting
        Refraction += saturate(lightScattering * _TranslucencyColor * _TranslucencyIntensity) * ( 1 - Foam.a)
* (1 - FogDensity * FogDensity);
    ...
    //  Specular Lighting
        ...
        specularLighting *= 1.0 - edgeBlendFactor;
    //  Add reflections
        #if defined (UNITY_PASS_FORWARDBASE)
